...
public class Code_1_1_BubbleSort {
	public static void bubbleSort1(int[] arr) {
		if (arr == null || arr.length == 1)
			return;
		for (int end = arr.length - 1; end > 0; end--) { // 此处是计算冒泡的次数，一共arr.length - 1次，因为最后一次只剩下一个数，这个数其实是已经排好序的
			for (int i = 0; i < end; i++) { // 除开排好序的尾部，再在剩下的数字中进行冒泡排序
				if (arr[i] > arr[i + 1]) { // 如果前一个数小于后一个数，则交换位置，当对数组中每一个数进行这步操作的时候，最大值一定会被交换在最后
					swap(arr, i, i + 1); // 注意：这个交换方式不能是交换自己，如果是交换自己的话，结果会成零。而可以交换两个相同的数
				}
			}
		}
	}
